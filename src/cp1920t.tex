...
\just={lei-3 ; lei-75}
%
  \ensuremath{\begin{lcbr}\Varid{maisDir}\comp \Conid{Empty}\mathrel{=}\Varid{g1}\;()\\\Varid{maisDir}\comp \Conid{Node}\;(\Varid{a},(\Varid{t1},\Varid{t2}))\mathrel{=}\Varid{g2}\comp (\Varid{a},((\Varid{maisDir}\;\Varid{t1}),(\Varid{maisDir}\;\Varid{t2})))\end{lcbr}}
%
 \end{eqnarray*}
...
Para g2 temos um contratempo pois não podemos apenas dizer que o resultado seria o obtido em maisDir t1 pois este pode retornar Nothing mas um simples pattern match resolve essa situação.
...
&
	\ensuremath{\Conid{A}\mathbin{+}\Conid{A}\times(\mathsf{LTree}\;^{Bool^*}\times\mathsf{LTree}\;^{Bool^*})}
		\ar[l]^-{\ensuremath{\Varid{g}\mathrel{=}\alt{\underline{\Conid{Leaf}}}{\Varid{\Conid{Cp}.cond}\;\Varid{null}\;(\Conid{Fork}\comp \conj{\Varid{t1}}{\Varid{t2}})\;(\Varid{\Conid{Cp}.cond}\;\Varid{head}\;(\Varid{t1}\comp \Varid{tail})\;(\Varid{t2}\comp \Varid{tail}))}}}
}
\end{eqnarray*}
